...
class Simple_Job_Board_Settings_General {
    ...
    public function sjb_save_settings_section() {
        ...
        if (!empty($jobpost_slug) || !empty($job_category_slug) || !empty($job_type_slug) || !empty($job_location_slug)) {

            // Save Custom Post Type Slug in WP Option
            (!empty($jobpost_slug) ) ? update_option('job_board_jobpost_slug', $jobpost_slug) : '';

            // Save Category Taxonomy Slug in WP Option
            (!empty($job_category_slug) ) ? update_option('job_board_job_category_slug', $job_category_slug) : '';

            // Save Job Type Taxonomy Slug in WP Option
            (!empty($job_type_slug) ) ? update_option('job_board_job_type_slug', $job_type_slug) : '';

            // Save Job Location Taxonomy Slug in WP Option
            (!empty($job_location_slug) ) ? update_option('job_board_job_location_slug', $job_location_slug) : '';
        }
    }
